...
router.get('/', function(req, res) {
  ...
  if(req.session.user){
    classification.all_classification(function(err, result){
      if(err){
        res.render('index', { title: 'Express' , user:JSON.stringify(req.session.user), classification_list:"", blog_list:""});
      }else{
        blog.all_blog(function(err, blog_list_result){
          if(err){
            res.render('index', { title: 'Express' , user:JSON.stringify(req.session.user), classification_list:JSON.stringify(result),blog_list:"" });
          }else{
            ...
            ep.after('author_portrait', blog_list_result.length, function(portraits){
              ...
              res.render('index', { title: 'Express' , user:JSON.stringify(req.session.user), classification_list:JSON.stringify(result),blog_list:JSON.stringify(blog_list_result) });
            });
            var portrait = [];
            for(var i = 0; i < blog_list_result.length; ++i){
                var c = blog_list_result[i];
